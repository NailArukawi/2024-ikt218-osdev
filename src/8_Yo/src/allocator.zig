...
pub const Block = extern struct {
    ...
    pub fn alloc(this: *@This(), len: usize) ?[*]u8 {
        ...
        vmm.pageMap(@intFromPtr(&this.body[room]), @intFromPtr(&this.body[room])) catch unreachable;
        vmm.switchPageDirectory(@intFromPtr(&vmm.page_directory[0]));
...
pub const KernelAllocator = struct { // todo support larger than 127 line allocations
    ...
    pub fn free(this: *@This(), ptr: anytype, len: usize) void {
        ...
        tty.print("free: (0x{x}) from block: (0x{x})\n", .{ @intFromPtr(ptr), block_base });
        block.free(len);
    }
};

pub fn printMemoryLayout(allocator: *KernelAllocator) void {
    tty.print("Kernel (start): \t0x{x}\n", .{@intFromPtr(&pmm._kernel_start)});
    tty.print("vmm (PD start):  \t0x{x} <block: not block allocated!>\n", .{@intFromPtr(&vmm.page_directory[0])});
    tty.print("vmm (PD end):   \t0x{x} <block: not block allocated!>\n", .{@intFromPtr(&vmm.page_directory[1023])});
    tty.print("Kernel (end):   \t0x{x}\n", .{@intFromPtr(&pmm._kernel_end)});
    tty.print("pmm (stack start): \t0x{x} <block: {}>\n", .{ @intFromPtr(&pmm.mem.free[0]), @intFromPtr(&pmm.mem.free) / 4096 });
    tty.print("pmm (stack end): \t0x{x} <block: {}>\n", .{ @intFromPtr(&pmm.mem.free[pmm.mem.free.len - 1]), @intFromPtr(&pmm.mem.free[pmm.mem.free.len - 1]) / 4096 });
    for (&vmm.page_directory, 0..) |*pd, i| {
        if (pd.present) {
            const table = &pd.get()[i];
            tty.print("vmm (PT{}): \t0x{x} <block: {}>\n", .{ i, @intFromPtr(table), @intFromPtr(table) / 4096 });
        }
    }

    tty.print("allocator blocks:\n", .{});
    for (&allocator.blocks, 0..) |b, i| {
        if (b != null) {
            tty.print("\t(Block{}): \t0x{x} <block: {}>\n", .{ i, @intFromPtr(b), @intFromPtr(b) / 4096 });
        }
    }
}
